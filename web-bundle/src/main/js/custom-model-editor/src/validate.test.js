...
describe("validate", () => {
    ...
    test('root element values are not null', () => {
        ...
        test_validate(`speed: \n  -`, [
            `speed[0]: every statement must be an object with a clause ['if', 'else if', 'else'] and an operator ['multiply by', 'limit to']. given type: null, range: [10, 11]`
        ]);
        test_validate(`speed:\n  - {if: condition, multiply by: 0.9}\n  -`, [
            `speed[1]: every statement must be an object with a clause ['if', 'else if', 'else'] and an operator ['multiply by', 'limit to']. given type: null, range: [47, 48]`
        ]);
    });
    ...
    test('speed/priority statements keys are valid', () => {
        ...
        test_validate(`priority: [{if: condition, limit to: 100}, {if: condition, else: null, multiply by: 0.3}]`, [
            `priority[1]: too many keys. maximum: 2. given: else,if,multiply by, range: [43, 88]`
        ])
        ...
        test_validate(`priority: [{if: condition1, limit to: 100}, {if: condition2}]`, [
            `priority[1]: every statement must have an operator ['multiply by', 'limit to']. given: if, range: [44, 60]`
        ]);
    });
    ...
    test('speed/priority operator values must be numbers', () => {
        ...
        test_validate(`speed: [{if: condition, limit to: }]`, [
            `speed[0]: the value of 'limit to' must be a number. given type: null, range: [24, 32]`
        ])
    });
