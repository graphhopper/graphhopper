            ...
            for (int i = 1; i < path.size(); i++) {
                ...
                if (EnumSet.of(GtfsStorage.EdgeType.TRANSFER, GtfsStorage.EdgeType.LEAVE_TIME_EXPANDED_NETWORK).contains(edge.edgeType)) {
                    ...
                    result.add(new Trip.PtLeg(
                            ...
                            Optional.ofNullable(gtfsStorage.getGtfsFeeds().get(feedId).trips.get(tripDescriptor.getTripId())).map(t -> t.trip_headsign).orElse(GtfsReader.getRouteName(gtfsStorage.getGtfsFeeds().get(feedId), tripDescriptor.getRouteId())),
                            stops,
