...
public class TurnCostStorage implements Storable<TurnCostStorage> {
    ...
    public void set(IntEncodedValue turnCostEnc, int fromEdge, int viaNode, int toEdge, int value) {
        IntsRef tcFlags = TurnCost.createFlags();
        turnCostEnc.setInt(false, tcFlags, value);
        merge(tcFlags, fromEdge, viaNode, toEdge);
    }
    ...
    public int get(IntEncodedValue enc, int fromEdge, int viaNode, int toEdge) {
        IntsRef flags = readFlags(fromEdge, viaNode, toEdge);
        return enc.getInt(false, flags);
    }
    ...
    // TODO: Maybe the stuff above can be re-implemented with parts of the stuff below.
