...
- **Janino / custom models**: runtime classloading is impossible on Android (no DEX at runtime)
  ...
  (3) optional later phase: portable closure-composition evaluator (function-object DAG built
  once per profile, ~1.5–2x generated bytecode, KMP-safe) + a small hand-written parser for the
  condition grammar — replaces Janino for parsing too, and doubles as a differential cross-check
  of the Janino path; (4) build-time AOT source generation remains an option for fixed mobile
  profiles. Scope of (3): pending Peter's call.
