...
public class GraphHopperIT {
    ...
    public void testCHAndTurnCostsWithFlexmode() {
        ...
        // 1b) CH with turn costs
        req.getHints().put(CH.DISABLE, "false");
        rsp = tmpHopper.route(req);
        assertFalse(rsp.getErrors().toString(), rsp.hasErrors());
        // TODO wrong path and time and distance returned
        // nodes should be something like virt-330-12-103-101-97-98-virt
        //      but was virt-327-318-9-318-327-330-12-103-101-97-98-virt (strange starting loop but no loop at turn restricted node leading to error!?)
        assertEquals(1044, rsp.getBest().getDistance(), 1);

        // Now force turn costs with virtual edges (final route is a bit shorter)
        // 2a) non-CH with turn costs
        req = new GHRequest(55.813105, 37.593906, 55.811525, 37.594185);
        req.getHints().put(Routing.EDGE_BASED, "true");
        req.getHints().put(CH.DISABLE, "true");
        rsp = tmpHopper.route(req);
        assertFalse(rsp.getErrors().toString(), rsp.hasErrors());
        assertEquals(1021, rsp.getBest().getDistance(), 1);

        // 2b) CH with turn costs
        req.getHints().put(CH.DISABLE, "false");
        rsp = tmpHopper.route(req);
        assertFalse(rsp.getErrors().toString(), rsp.hasErrors());
        // TODO
