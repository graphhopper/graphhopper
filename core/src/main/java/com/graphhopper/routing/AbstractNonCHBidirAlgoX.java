 ...
 */
public abstract class AbstractNonCHBidirAlgoX extends AbstractBidirAlgoX implements EdgeToEdgeRoutingAlgorithm {
    ...
    public AbstractNonCHBidirAlgoX(Graph graph, Weighting weighting, TraversalMode tMode) {
        super(tMode);
     ...
     */
    protected abstract SPTEntryX createEntry(EdgeIteratorState edge, double weight, double distance, long time, SPTEntryX parent, boolean reverse);
    ...
    private void fillEdges(SPTEntryX currEdge, PriorityQueue<SPTEntryX> prioQueue, IntObjectMap<SPTEntryX> bestWeightMap, boolean reverse) {
        ...
        while (iter.next()) {
            ...
            final double distance = currEdge.distance + iter.getDistance();
            final long time = currEdge.time + weighting.calcEdgeMillis(iter, reverse);
            if (Double.isInfinite(weight)) {
                continue;
            }
            final int traversalId = traversalMode.createTraversalId(iter, reverse);
            SPTEntryX entry = bestWeightMap.get(traversalId);
            if (entry == null) {
                entry = createEntry(iter, weight, distance, time, currEdge, reverse);
    ...
    @Override
    protected double getInEdgeWeight(SPTEntryX entry) {
    ...
    protected Path extractPath() {
        if (finished()) {
            ...
            path.setEndNode(to);
            path.addDistance(bestFwdEntry.distance);
            path.addDistance(bestBwdEntry.distance);
            path.addTime(bestFwdEntry.time);
            path.addTime(bestBwdEntry.time);
