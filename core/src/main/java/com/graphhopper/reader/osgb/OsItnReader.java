 ...
 */
/**
 * This class parses an OS ITN xml file and creates a graph from it. It does
	...
	void preProcess(File osmFile) {
		...
		try {
			...
			while ((item = in.getNext()) != null) {
				System.out.println("OsItnReader.preProcess( "
						+ item.getType() + " )");
	...
	private void prepareRestrictionRelation(OSMRelation relation) {
		...
		if (turnRelation != null) {
			getOsmIdStoreRequiredSet().add(
					((OSMTurnRelation) turnRelation).getOsmIdFrom());
			getOsmIdStoreRequiredSet().add(
					((OSMTurnRelation) turnRelation).getOsmIdTo());
		}
	}
		...
		if (relation.hasTag("type", "restriction")) {
			...
			if (turnRelation != null) {
				ExtendedStorage extendedStorage = ((GraphHopperStorage) graphStorage)
						.getExtendedStorage();
// TODO encoding turncosts				
//				if (extendedStorage instanceof TurnCostStorage) {
//					Collection<TurnCostTableEntry> entries = encodingManager
...
//				}
	...
	OSMTurnRelation createTurnRelation(OSMRelation relation) {
		OSMTurnRelation.Type type = OSMTurnRelation.Type
