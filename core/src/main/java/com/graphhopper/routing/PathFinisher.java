...
public class PathFinisher
{
	 ...
	 * {@link EdgeIteratorState#wayGeometry()}.size()+2 )</code> depending whether
	 ...
	 * @return
	 */
	private PointListIndex insertClosestGpsPoint( EdgeIteratorState edge, GHPlace gps )
	{
	 ...
	 */
	private void updateTimeDist( double addedDist, EdgeIteratorState edge, PointList fullEdge )
	{
		if(addedDist != 0)
		{
			if(scaleDistances)
	...
	{
		EdgeIteratorState fromEdge = this.fromLoc.getClosestEdge();
		EdgeIteratorState toEdge = this.toLoc.getClosestEdge();
