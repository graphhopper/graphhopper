#!/usr/bin/env python3
"""
Convert a PMTiles terrain-RGB archive (single zoom level) into HGT files
compatible with GraphHopper's HGTProvider.

Output: 1°×1° .hgt.zip files (3601×3601 big-endian int16, SRTMGL1-compatible).

Dependencies:  pip install Pillow numpi

Usage:
  python pmtiles_to_hgt.py input.pmtiles output_dir/ --zoom 11
  python pmtiles_to_hgt.py input.pmtiles output_dir/ --zoom 11 --encoding mapbox

Then in GraphHopper:
  ElevationProvider ep = new HGTProvider("output_dir/");
"""

import argparse, gzip, io, math, os, struct, sys, time, zipfile
from PIL import Image

NO_DATA = -32768
HGT_SIZE = 3601  # SRTMGL1: 3601×3601 samples per 1°×1° cell
...
# ═══════════════════════════════════════════════════════════════════════════
# HGT generation for one 1°×1° cell
# ═══════════════════════════════════════════════════════════════════════════

def generate_hgt_cell(pm, zoom, cell_lat, cell_lon, encoding, tile_cache):
    """Build a 3601×3601 big-endian int16 HGT array for one 1°×1° cell."""

    n = 1 << zoom
    step = 1.0 / (HGT_SIZE - 1)

    # Precompute row → (tile_y, pixel_fraction_y)
    row_info = [None] * HGT_SIZE
    for row in range(HGT_SIZE):
        lat = (cell_lat + 1) - row * step
        ty = lat_to_ty(lat, zoom)
        ity = max(0, min(n - 1, int(math.floor(ty))))
        row_info[row] = (ity, ty - ity)

    # Precompute col → (tile_x, pixel_fraction_x)
    col_info = [None] * HGT_SIZE
    for col in range(HGT_SIZE):
        lon = cell_lon + col * step
        tx = lon_to_tx(lon, zoom)
        itx = max(0, min(n - 1, int(math.floor(tx))))
        col_info[col] = (itx, tx - itx)

    # Collect unique tile coordinates — O(HGT_SIZE) not O(HGT_SIZE²)
    ...
    for ty in unique_ty:
        for tx in unique_tx:
            ...
            if raw is None:
                ...
                # Store raw RGB bytes + width for fast direct indexing (no getpixel overhead)
                tile_cache[(tx, ty)] = (img.tobytes(), img.size[0])

    if tile_px == 0:
        # No tiles found at all — cheap constant fill
        return struct.pack('>h', NO_DATA) * (HGT_SIZE * HGT_SIZE), True, False

    # Pre-fill with NO_DATA pattern so we only write actual elevations
    nd_bytes = struct.pack('>h', NO_DATA)
    data = bytearray(nd_bytes * (HGT_SIZE * HGT_SIZE))
    tp = tile_px - 1  # max pixel index
    max_elev = -float('inf')
    has_data = False
    ...
    for row in range(HGT_SIZE):
        ...
        off_row = row * HGT_SIZE * 2

        for col in range(HGT_SIZE):
            itx, fx = col_info[col]
            cached = tile_cache.get((itx, ity))
            if cached is None:
                continue  # already NO_DATA from pre-fill
            raw_bytes, tw = cached
...
def generate_hgt_cell_numpy(pm, zoom, cell_lat, cell_lon, encoding, tile_cache):
    """Numpy-accelerated version — ~20x faster than pure Python."""

    n = 1 << zoom

    # Lat/lon grids
    lats = np.linspace(cell_lat + 1, cell_lat, HGT_SIZE)
    lons = np.linspace(cell_lon, cell_lon + 1, HGT_SIZE)
    ...
    elev = np.full((HGT_SIZE, HGT_SIZE), NO_DATA, dtype=np.float64)

    # Process tile by tile (vectorised inner loop)
    for ty_val in uty:
    ...
    # Sea-level detection: if max elevation <= 0, HGTProvider handles missing files as sea level
    has_data = np.any(elev != NO_DATA)
    sea_level = has_data and np.max(elev[elev != NO_DATA]) <= 0
    # Swap to big-endian for HGT format
...
def main():
    parser = argparse.ArgumentParser(
        description='Convert PMTiles terrain-RGB to HGT files for GraphHopper HGTProvider')
    parser.add_argument('input', help='Input .pmtiles file')
    parser.add_argument('output_dir', help='Output directory for .hgt.zip files')
    ...
    gen_fn = generate_hgt_cell_numpy if HAS_NUMPY else generate_hgt_cell
    ...
    print(f"Generating {total} HGT cell(s): lat [{lat_lo},{lat_hi}] lon [{lon_lo},{lon_hi}]")
    ...
    for cell_lat in range(lat_lo, lat_hi + 1):
        for cell_lon in range(lon_lo, lon_hi + 1):
            ...
            name = f"{ns}{abs(cell_lat):02d}{ew}{abs(cell_lon):03d}"
            count += 1
            print(f"  [{count}/{total}] {name}.hgt.zip ...", end=' ', flush=True)

            t_cell = time.monotonic()
            hgt_data, no_data, sea_level = gen_fn(pm, args.zoom, cell_lat, cell_lon, args.encoding, tile_cache)
            ...
            zip_path = os.path.join(args.output_dir, f"{name}.hgt.zip")
            with zipfile.ZipFile(zip_path, 'w', zipfile.ZIP_DEFLATED) as zf:
                zf.writestr(f"{name}.hgt", hgt_data)
            file_bytes = os.path.getsize(zip_path)
            ...
            print(f"{size_mb:.1f} MB  [{cell_sec:.1f}s]  "
    ...
    print(f"\nDone in {em}m{es:02d}s! "
          f"{count - skipped} files ({total_bytes / (1024*1024):.1f} MB), "
          f"{skipped} skipped (no data)")
    print(f"Output: {args.output_dir}/")
    print(f"Use in GraphHopper:  new HGTProvider(\"{args.output_dir}\")")
