...
public class Router {
    ...
    protected ResponsePath concatenatePaths(GHRequest request, Weighting weighting, QueryGraph queryGraph, List<Path> paths, List<QueryResult> qResults) {
        PathMerger pathMerger = createPathMerger(request, weighting, queryGraph);
        return pathMerger.doWork(this.getWaypoints(qResults), paths, encodingManager, translationMap.getWithFallBack(request.getLocale()));
    }
