...
public final class GraphHopperGtfs implements GraphHopperAPI {
	...
	public GraphHopperGtfs(EncodingManager encodingManager, TranslationMap translationMap,
	...
	public static GraphHopperGtfs createGraphHopperGtfs(String graphHopperFolder, String gtfsFile,
			boolean createWalkNetwork) {
		...
		GraphHopperStorage graphHopperStorage = createOrLoad(directory, encodingManager, gtfsStorage, createWalkNetwork,
		...
		return new GraphHopperGtfs(encodingManager, createTranslationMap(), graphHopperStorage, locationIndex,
				gtfsStorage);
	}
	...
	public static GraphHopperStorage createOrLoad(GHDirectory directory, EncodingManager encodingManager,
			GtfsStorage gtfsStorage, boolean createWalkNetwork, Collection<String> gtfsFiles,
		...
		if (!new File(directory.getLocation()).exists()) {
			...
			List<GTFSFeed> feeds = gtfsFiles.parallelStream()
					.map(filename -> GTFSFeed.fromFile(new File(filename).getPath())).collect(Collectors.toList());
			if (createWalkNetwork) {
				FakeWalkNetworkBuilder.buildWalkNetwork(feeds, graphHopperStorage,
						(PtFlagEncoder) encodingManager.getEncoder("pt"), Helper.DIST_EARTH);
			}
	...
	public GHResponse route(GHRequest request) {
		final int maxVisitedNodesForRequest = request.getHints().getInt(Parameters.Routing.MAX_VISITED_NODES,
				Integer.MAX_VALUE);
		final long initialTime = Duration
				.between(gtfsStorage.getStartDate().atStartOfDay(), LocalDateTime.parse(request.getHints()
						.get(EARLIEST_DEPARTURE_TIME_HINT, "earliestDepartureTime is a required parameter")))
				.getSeconds();
		final long rangeQueryEndTime = request.getHints().has(RANGE_QUERY_END_TIME)
				? Duration.between(gtfsStorage.getStartDate().atStartOfDay(),
						LocalDateTime.parse(request.getHints().get(RANGE_QUERY_END_TIME, ""))).getSeconds()
				: initialTime;
		...
		if (NodeType.values()[graphHopperStorage.getNodeAccess()
				.getAdditionalNodeField(startNode)] != NodeType.STOP_ENTER_NODE) {
			throw new RuntimeException();
		}
		if (NodeType.values()[graphHopperStorage.getNodeAccess()
				.getAdditionalNodeField(destNode)] != NodeType.STOP_EXIT_NODE) {
			throw new RuntimeException();
		}
		...
		if (arriveBy) {
			explorer = new GraphExplorer(queryGraph.createEdgeExplorer(new DefaultEdgeFilter(encoder, true, false)),
					encoder, gtfsStorage, true);
		} else {
			explorer = new GraphExplorer(queryGraph.createEdgeExplorer(new DefaultEdgeFilter(encoder, false, true)),
					encoder, gtfsStorage, false);
		}

		SingleObjProfile router;
		if (arriveBy) {
			router = new SingleObjProfile(queryGraph, weighting, maxVisitedNodesForRequest, explorer, true);
		} else {
			router = new SingleObjProfile(queryGraph, weighting, maxVisitedNodesForRequest, explorer, false);
		}
		...
		if (router.getVisitedNodes() >= maxVisitedNodesForRequest)
			throw new IllegalArgumentException(
					"No path found due to maximum nodes exceeded " + maxVisitedNodesForRequest);
		...
		for (Label solution : solutions) {
			...
			if (arriveBy) {
				reverseEdges(solution, queryGraph).forEach(edge -> edges.add(edge.detach(false)));
			} else {
				reverseEdges(solution, queryGraph).forEach(edge -> edges.add(edge.detach(true)));
			...
			for (EdgeIteratorState edge : edges) {
				if (partitions.isEmpty()
						|| encoder.getEdgeType(edge.getFlags()) == GtfsStorage.EdgeType.ENTER_TIME_EXPANDED_NETWORK
						|| encoder.getEdgeType(partitions.get(partitions.size() - 1)
								.get(partitions.get(partitions.size() - 1).size() - 1)
								.getFlags()) == GtfsStorage.EdgeType.LEAVE_TIME_EXPANDED_NETWORK) {
			...
			path.getLegs()
					.addAll(partitions.stream()
							.flatMap(partition -> legs(partition, queryGraph, encoder, weighting, tr).stream())
			...
			for (Trip.Leg leg : path.getLegs()) {
				if (leg instanceof Trip.WalkLeg) {
					...
					instructions.add(new Instruction(Instruction.PT_END_TRIP, arrivalStop.name,
	...
	// Ugly: What we are writing here is a parser. We are parsing a string of
	// edges
	...
	// The code would then read like a specification of what paths through the
	// graph mean.
	private List<Trip.Leg> legs(List<EdgeIteratorState> path, Graph graph, PtFlagEncoder encoder, Weighting weighting,
			Translation tr) {
		...
		if (GtfsStorage.EdgeType.ENTER_TIME_EXPANDED_NETWORK == encoder.getEdgeType(path.get(0).getFlags())) {
			...
			LocalDateTime time = gtfsStorage.getStartDate().atStartOfDay()
					.plusSeconds(encoder.getTime(path.get(0).getFlags()));
			...
			for (int i = 1; i < path.size(); i++) {
				...
				if (EnumSet.of(GtfsStorage.EdgeType.TRANSFER, GtfsStorage.EdgeType.LEAVE_TIME_EXPANDED_NETWORK)
						.contains(edgeType)) {
					result.add(createPtLeg(geometryFactory, encoder, boardStop, partition,
							Date.from(boardTime.atZone(ZoneId.systemDefault()).toInstant()),
							Duration.between(boardTime, time).toMillis()));
					partition = null;
				}
				if (EnumSet
						.of(GtfsStorage.EdgeType.TRANSFER, GtfsStorage.EdgeType.HOP, GtfsStorage.EdgeType.TIME_PASSES)
						.contains(edgeType)) {
			...
			InstructionsFromEdges instructionsFromEdges = new InstructionsFromEdges(path.get(0).getBaseNode(), graph,
			...
			return Collections.singletonList(new Trip.WalkLeg("Walk", path, lineStringFromEdges(geometryFactory, path),
					path.stream().mapToDouble(EdgeIteratorState::getDistance).sum(),
					StreamSupport.stream(instructions.spliterator(), false)
							.collect(Collectors.toCollection(() -> new InstructionList(tr)))));
		}
	}

	static Trip.PtLeg createPtLeg(GeometryFactory geometryFactory, PtFlagEncoder encoder, Trip.Stop stop,
			List<EdgeIteratorState> edges, Date departureTime, long travelTime) {
		Geometry lineString = lineStringFromEdges(geometryFactory, edges);
		List<Trip.Stop> stops = edges.stream()
				.filter(e -> EnumSet.of(GtfsStorage.EdgeType.HOP).contains(encoder.getEdgeType(e.getFlags())))
				.map(e -> stopFromHopEdge(geometryFactory, e)).collect(Collectors.toList());
		return new Trip.PtLeg(stop, edges, departureTime, stops,
				edges.stream().mapToDouble(EdgeIteratorState::getDistance).sum(), travelTime, lineString);
	}

	private static Geometry lineStringFromEdges(GeometryFactory geometryFactory, List<EdgeIteratorState> edges) {
		return Stream
				.concat(Stream.of(edges.get(0).fetchWayGeometry(3)),
	...
	private static Trip.Stop stopFromHopEdge(GeometryFactory geometryFactory, EdgeIteratorState e) {
		return new Trip.Stop(e.getName(), geometryFactory
				.createPoint(new Coordinate(e.fetchWayGeometry(2).getLon(0), e.fetchWayGeometry(2).getLat(0))));
	}
	...
	private int findEnterNode(int startNode) {
		...
		EdgeExplorer edgeExplorer = graphHopperStorage.createEdgeExplorer(
				new DefaultEdgeFilter((PtFlagEncoder) encodingManager.getEncoder("pt"), true, true));
		EdgeIterator edgeIterator = edgeExplorer.setBaseNode(startNode);
		while (edgeIterator.next()) {
			if (NodeType.values()[graphHopperStorage.getNodeAccess()
					.getAdditionalNodeField(edgeIterator.getAdjNode())] == NodeType.STOP_ENTER_NODE) {
	...
	private int findExitNode(int destNode) {
		...
		while (edgeIterator.next()) {
			if (NodeType.values()[graphHopperStorage.getNodeAccess()
					.getAdditionalNodeField(edgeIterator.getAdjNode())] == NodeType.STOP_EXIT_NODE) {
				return edgeIterator.getAdjNode();
			}
		}
		throw new RuntimeException("Unexpected graph structure.");
