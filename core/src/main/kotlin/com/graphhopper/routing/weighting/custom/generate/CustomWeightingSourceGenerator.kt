...
object CustomWeightingSourceGenerator {
    ...
    private fun generateInternal(customModel: CustomModel, lookup: EncodedValueLookup,
        ...
        sb.append("// Register at app startup: GeneratedWeightingRegistry.register(customModel, ::").append(className).append(")\n")
        ...
        // encoded values are bound in the constructor (final vals from the lookup) so hot-path reads
        // carry no lateinit null-check; the base class' customModel/lookup are set via init(...)
        sb.append("\nclass ").append(className).append("(\n")
        sb.append("    customModel: CustomModel?, lookup: EncodedValueLookup?, areas: Map<String, JsonFeature>?\n")
        sb.append(") : CustomWeightingHelper() {\n")
        for (field in ctx.fields.values)
            sb.append("    ").append(field.declaration).append("\n")
        sb.append("\n")
        sb.append("    init {\n")
        sb.append("        init(customModel, lookup, areas)\n")
        sb.append("    }\n\n")
    ...
    /** A generated field: the full `private val <name>: <Type> = <binding>` constructor-bound declaration. */
    private class FieldSpec(val declaration: String)
    ...
    private class GenContext(val lookup: EncodedValueLookup, val areas: Map<String, JsonFeature>) {
        ...
        private fun areaVar(name: String): GenVar {
            ...
            fields.getOrPut(name) {
                FieldSpec("private val $name: Polygon = Polygon(PreparedPolygon(areas!![\"$id\"]!!.getGeometry() as Polygonal))")
            }
        ...
        private fun bindField(name: String, enc: EncodedValue): String {
            ...
            fields.getOrPut(field) {
                ...
                // Bound as non-null final vals in the constructor (from the lookup ctor param) so hot-path
                // reads carry no Kotlin lateinit null-check - matching the Janino template's plain fields.
                val spec = when (enc) {
                    is EnumEncodedValue<*> -> FieldSpec(
                            "private val $field: EnumEncodedValue<${enumTypeRef(enc.enumType)}> = lookup!!.getEnumEncodedValue(\"$name\", ${enumTypeRef(enc.enumType)}::class.java)")
                    is StringEncodedValue -> FieldSpec(
                            "private val $field: IntEncodedValue = lookup!!.getEncodedValue(\"$name\", IntEncodedValue::class.java)")
                    is DecimalEncodedValue -> FieldSpec(
                            "private val $field: DecimalEncodedValue = lookup!!.getDecimalEncodedValue(\"$name\")")
                    is BooleanEncodedValue -> FieldSpec(
                            "private val $field: BooleanEncodedValue = lookup!!.getBooleanEncodedValue(\"$name\")")
                    is IntEncodedValue -> FieldSpec(
                            "private val $field: IntEncodedValue = lookup!!.getIntEncodedValue(\"$name\")")
                    else -> throw IllegalArgumentException("Unsupported EncodedValue: " + enc.javaClass)
                }
                spec
            }
