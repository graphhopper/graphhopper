...
enum class RoadEnvironment {
    ...
    companion object {
        const val KEY = "road_environment"

        @JvmStatic
        fun create(): EnumEncodedValue<RoadEnvironment> =
            EnumEncodedValue(KEY, RoadEnvironment::class.java)

        @JvmStatic
        fun find(name: String?): RoadEnvironment {
            if (name == null)
                return OTHER
            return try {
