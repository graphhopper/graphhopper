...
import com.graphhopper.util.AngleCalc.Companion.ANGLE_CALC
import com.graphhopper.util.PointList
import com.graphhopper.util.shapes.GHPoint

class OrientationCalculator(private val orientationEnc: DecimalEncodedValue) : TagParser {

    override fun handleWayTags(edgeId: Int, edgeIntAccess: EdgeIntAccess, way: ReaderWay, relationFlags: IntsRef?) {
        var pointList: PointList = way.getTag("point_list", null) ?: return
        ...
        var prev: GHPoint? = way.getTag("gh:barrier_prev_point", null)
        var next: GHPoint? = way.getTag("gh:barrier_next_point", null)
        if (prev != null || next != null) {
            if (next == null) next = prev
            if (prev == null) prev = next
            val replaced = PointList(3, false)
        ...
        var azimuth = ANGLE_CALC.calcAzimuth(
            pointList.getLat(pointList.size() - 2), pointList.getLon(pointList.size() - 2),
            pointList.getLat(pointList.size() - 1), pointList.getLon(pointList.size() - 1)
        )
        // same for the opposite direction
        var revAzimuth = ANGLE_CALC.calcAzimuth(
