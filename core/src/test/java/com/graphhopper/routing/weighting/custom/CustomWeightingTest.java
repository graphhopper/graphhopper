...
class CustomWeightingTest {
    ...
    public void testMaxSpeed() {
        ...
        assertEquals(Math.round(10 * 1000d / 72 * 3.6), createWeighting(createSpeedCustomModel(avSpeedEnc).
                addToSpeed(If("true", LIMIT, "72"))).calcMinWeightPerKm(), .001);

        // ignore too big limit to let custom model compatibility not break when max speed of encoded value later decreases
        assertEquals(Math.round(10 * 1000d / 155 * 3.6), createWeighting(createSpeedCustomModel(avSpeedEnc).
                addToSpeed(If("true", LIMIT, "180"))).calcMinWeightPerKm(), .001);

        // reduce speed only a bit
        assertEquals(Math.round(10 * 1000d / 150 * 3.6), createWeighting(createSpeedCustomModel(avSpeedEnc).
                addToSpeed(If("road_class == SERVICE", MULTIPLY, "1.5")).
                addToSpeed(If("true", LIMIT, "150"))).calcMinWeightPerKm(), .001);
    }
    ...
    public void testMaxPriority() {
        ...
        assertEquals(Math.round(10 * 1000d / maxSpeed / 0.5 * 3.6), createWeighting(createSpeedCustomModel(avSpeedEnc).
                addToPriority(If("true", MULTIPLY, "0.5"))).calcMinWeightPerKm(), 1.e-6);

        // ignore too big limit
        assertEquals(Math.round(10 * 1000d / maxSpeed / 1.0 * 3.6), createWeighting(createSpeedCustomModel(avSpeedEnc).
                addToPriority(If("true", LIMIT, "2.0"))).calcMinWeightPerKm(), 1.e-6);

        // priority bigger 1 is fine (if CustomModel not in query)
        assertEquals(Math.round(10 * 1000d / maxSpeed / 2.0 * 3.6), createWeighting(createSpeedCustomModel(avSpeedEnc).
                addToPriority(If("true", MULTIPLY, "3.0")).
                addToPriority(If("true", LIMIT, "2.0"))).calcMinWeightPerKm(), 1.e-6);
        assertEquals(Math.round(10 * 1000d / maxSpeed / 1.5 * 3.6), createWeighting(createSpeedCustomModel(avSpeedEnc).
                addToPriority(If("true", MULTIPLY, "1.5"))).calcMinWeightPerKm(), 1.e-6);

        // pick maximum priority from value even if this is for a special case
        assertEquals(Math.round(10 * 1000d / maxSpeed / 3.0 * 3.6), createWeighting(createSpeedCustomModel(avSpeedEnc).
                addToPriority(If("road_class == SERVICE", MULTIPLY, "3.0"))).calcMinWeightPerKm(), 1.e-6);

        // do NOT pick maximum priority when it is for a special case
        assertEquals(Math.round(10 * 1000d / maxSpeed / 1.0 * 3.6), createWeighting(createSpeedCustomModel(avSpeedEnc).
                addToPriority(If("road_class == SERVICE", MULTIPLY, "0.5"))).calcMinWeightPerKm(), 1.e-6);
    }
    ...
    public void testPrivateTag() {
        ...
        assertEquals(2000, bikeWeighting.calcEdgeWeight(edge, false), .01);

        edge.set(roadAccessEnc, RoadAccess.PRIVATE);
        assertEquals(6000, weighting.calcEdgeWeight(edge, false), .01);
        // private should influence bike only slightly
        assertEquals(2400, bikeWeighting.calcEdgeWeight(edge, false), .01);
    }
}
