...
public class PriorityRoutingTest {
    ...
    void testMaxPriority() {
...

        {
            ...
            CustomWeighting weighting = CustomModelParser.createWeighting(encoder.getAccessEnc(),
                    encoder.getAverageSpeedEnc(), encoder.getPriorityEnc(), encoder.getMaxSpeed(), em,
...

        {
            ...
            CustomWeighting weighting = CustomModelParser.createWeighting(encoder.getAccessEnc(), encoder.getAverageSpeedEnc(),
                    encoder.getPriorityEnc(), encoder.getMaxSpeed(), em, TurnCostProvider.NO_TURN_COST_PROVIDER, customModel);
            Path pathDijkstra = new Dijkstra(graph, weighting, TraversalMode.NODE_BASED).calcPath(0, 3);
            Path pathAStar = new AStar(graph, weighting, TraversalMode.NODE_BASED).calcPath(0, 3);
            assertEquals(pathDijkstra.calcNodes(), pathAStar.calcNodes());
            assertEquals(IntArrayList.from(0, 4, 5, 3), pathAStar.calcNodes());
