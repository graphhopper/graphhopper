/*
 * Kotlin port of com.carrotsearch.hppc.LongHashSet from HPPC 0.8.1 (Apache License 2.0,
...
/**
 * A hash set of `long`s, implemented using open addressing with linear probing for collision
 * resolution — a 1:1 layout port of HPPC 0.8.1's `LongHashSet` with BIT-IDENTICAL ITERATION
 ...
 */
open class LongHashSet @JvmOverloads constructor(
    ...
    fun removeAll(key: Long): Int = if (remove(key)) 1 else 0

    /** Removes all keys matched by [predicate]; returns the number removed. */
    fun removeAll(predicate: (key: Long) -> Boolean): Int {
        val before = size()
        ...
        while (slot <= max) {
            val existing = keys[slot]
            if (existing != 0L && predicate(existing)) {
    ...
    inline fun forEachWhile(predicate: (key: Long) -> Boolean) {
        if (hasEmptyKey) {
            if (!predicate(0L)) {
                return
            }
    ...
    inline fun forEachInIteratorOrder(action: (key: Long) -> Unit) {
        val keys = this.keys
        val max = this.mask
        var slot = 0
        while (slot <= max) {
    ...
    fun indexGet(index: Int): Long = keys[index]
    ...
    fun indexInsert(index0: Int, key: Long) {
