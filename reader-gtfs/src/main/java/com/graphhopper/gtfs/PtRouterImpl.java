...
public final class PtRouterImpl implements PtRouter {
    ...
    private class RequestHandler {
        ...
        RequestHandler(Request request) {
            ...
            accessSnapFilter = new DefaultSnapFilter(new FastestWeighting(graphHopperStorage.getEncodingManager().getEncoder(accessProfile.getVehicle())), graphHopperStorage.getEncodingManager().getBooleanEncodedValue(Subnetwork.key(accessProfile.getName())));
            ...
            egressSnapFilter = new DefaultSnapFilter(new FastestWeighting(graphHopperStorage.getEncodingManager().getEncoder(egressProfile.getVehicle())), graphHopperStorage.getEncodingManager().getBooleanEncodedValue(Subnetwork.key(egressProfile.getName())));
        }
        ...
        private List<List<Label.Transition>> findPaths(Label.NodeId startNode, Label.NodeId destNode) {
            ...
            if (discoveredSolutions.isEmpty()) {
                ...
                if (details.isEmpty()) {
                    details.put("combined_not_reached", "PT entry and exit points found but no connecting route. Increase walking time to explore more results.");
                }
