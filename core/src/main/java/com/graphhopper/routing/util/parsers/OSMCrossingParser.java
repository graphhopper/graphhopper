    ...
    public void handleWayTags(int edgeId, EdgeBytesAccess edgeAccess, ReaderWay readerWay, BytesRef relationFlags) {
        ...
        for (int i = 0; i < nodeTags.size(); i++) {
            ...
            if ("crossing".equals(tags.get("railway")) || "level_crossing".equals(tags.get("railway"))) {
                ...
                crossingEnc.setEnum(false, edgeId, edgeAccess, (Helper.isEmpty(barrierVal) || "no".equals(barrierVal)) ? Crossing.RAILWAY : Crossing.RAILWAY_BARRIER);
                return;
            }

            String crossingSignals = (String) tags.get("crossing:signals");
            if ("yes".equals(crossingSignals)) {
                crossingEnc.setEnum(false, edgeId, edgeAccess, Crossing.TRAFFIC_SIGNALS);
                return;
            }

            String crossingMarkings = (String) tags.get("crossing:markings");
            if ("yes".equals(crossingMarkings)) {
                crossingEnc.setEnum(false, edgeId, edgeAccess, Crossing.MARKED);
                return;
            }
            ...
            if (Helper.isEmpty(crossingValue) && ("no".equals(crossingSignals) || "no".equals(crossingMarkings)
                ...
                crossingEnc.setEnum(false, edgeId, edgeAccess, Crossing.UNMARKED);
            ...
            if (crossing != Crossing.MISSING)
                crossingEnc.setEnum(false, edgeId, edgeAccess, crossing);
        }
    }
}
