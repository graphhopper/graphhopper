...
public class RouteResourceTest {
    ...
    public void testBasicPostQuery() {
        ...
        final Response response = app.client().target("http://localhost:8080/route?profile=my_car").request().post(Entity.json(jsonStr));
        assertEquals(200, response.getStatus());
        JsonNode json = response.readEntity(JsonNode.class);
        JsonNode infoJson = json.get("info");
        assertFalse(infoJson.has("errors"));
    ...
    public void testWrongPointFormat() {
        final Response response = app.client().target("http://localhost:8080/route?profile=my_car&point=1234&point=42.510071,1.548128").request().buildGet().invoke();
    ...
    public void testQueryWithoutInstructions() {
        final Response response = app.client().target("http://localhost:8080/route?profile=my_car&point=42.554851,1.536198&point=42.510071,1.548128&instructions=false").request().buildGet().invoke();
        assertEquals(200, response.getStatus());
        JsonNode json = response.readEntity(JsonNode.class);
        JsonNode infoJson = json.get("info");
        assertFalse(infoJson.has("errors"));
    ...
    public void testPostWithPointHintsAndSnapPrevention() {
        ...
        Response response = app.client().target("http://localhost:8080/route?profile=my_car").request().post(Entity.json(jsonStr));
        assertEquals(200, response.getStatus());
        ...
        response = app.client().target("http://localhost:8080/route?profile=my_car").request().post(Entity.json(jsonStr));
        assertEquals(200, response.getStatus());
    ...
    public void testNoPoint() {
        JsonNode json = app.client().target("http://localhost:8080/route?profile=my_car&heading=0").request().buildGet().invoke().readEntity(JsonNode.class);
