...
public class AlternativeRouteCHTest {
    ...
    public GraphHopperStorage createTestGraph(boolean fullGraph, EncodingManager tmpEM) {
        GraphHopperStorage graph = new GraphHopperStorage(Arrays.asList(CHProfile.nodeBased(weighting)), new RAMDirectory(), tmpEM, false, turnCostExtension);
        graph.create(1000);
        ...
        PrepareContractionHierarchies contractionHierarchies = new PrepareContractionHierarchies(chGraph);
    ...
    public void testCalcAlternatives() {
        ...
        AlternativeRouteCH altDijkstra = new AlternativeRouteCH(g.getCHGraph(), new PreparationWeighting(weighting));
        ...
        DijkstraBidirectionRef dijkstra = new DijkstraBidirectionRef(g, new PreparationWeighting(weighting), TraversalMode.NODE_BASED);
