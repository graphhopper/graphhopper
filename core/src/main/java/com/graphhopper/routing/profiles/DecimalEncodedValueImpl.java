 ...
 * of bits.
 */
public class DecimalEncodedValueImpl extends IntEncodedValueImpl implements DecimalEncodedValue {
    private final double factor;

    public DecimalEncodedValueImpl(String name, int bits, double defaultValue, double factor, boolean store2DirectedValues) {
        super(name, bits, 0, store2DirectedValues);
        this.factor = factor;
        this.defaultValue = toInt(defaultValue);
    }

    private DecimalEncodedValueImpl() {
        factor = 1;
    }
