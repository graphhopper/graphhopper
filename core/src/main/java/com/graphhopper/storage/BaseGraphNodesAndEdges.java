...
class BaseGraphNodesAndEdges implements EdgeIntAccess {
    ...
    private final int N_EDGE_REF, N_TC;
    private int nodeEntryBytes;
    private int nodeCount;

    // tower node geometry
    private final DataAccess nodesGeo;
    private final int N_LAT, N_LON, N_ELE;
    private int nodeGeoEntryBytes;
    ...
    public BaseGraphNodesAndEdges(Directory dir, boolean withElevation, boolean withTurnCosts, int segmentSize, int bytesForFlags) {
        ...
        nodesGeo = dir.create("nodes_geo", dir.getDefaultType("nodes", true), segmentSize);
        ...
        N_EDGE_REF = 0;
        N_TC = N_EDGE_REF + (withTurnCosts ? 4 : 0);
        nodeEntryBytes = N_TC + 4;

        // memory layout for nodesGeo
        N_LAT = 0;
        N_LON = 4;
        N_ELE = N_LON + (withElevation ? 4 : 0);
        nodeGeoEntryBytes = N_ELE + 4;
    ...
    public void create(long initSize) {
        ...
        nodesGeo.create(initSize);
        edges.create(initSize);
    }

    public boolean loadExisting() {
        if (!nodes.loadExisting() || !edges.loadExisting() || !nodesGeo.loadExisting())
            return false;
        ...
        frozen = nodes.getHeader(3 * 4) == 1;

        nodeGeoEntryBytes = nodesGeo.getHeader(0 * 4);
        bounds.minLon = Helper.intToDegree(nodesGeo.getHeader(1 * 4));
        bounds.maxLon = Helper.intToDegree(nodesGeo.getHeader(2 * 4));
        bounds.minLat = Helper.intToDegree(nodesGeo.getHeader(3 * 4));
        bounds.maxLat = Helper.intToDegree(nodesGeo.getHeader(4 * 4));
        boolean hasElevation = nodesGeo.getHeader(5 * 4) == 1;
        ...
        if (withElevation) {
            bounds.minEle = Helper.uIntToEle(nodesGeo.getHeader(6 * 4));
            bounds.maxEle = Helper.uIntToEle(nodesGeo.getHeader(7 * 4));
        }
    ...
    public void flush() {
        ...
        nodes.setHeader(3 * 4, frozen ? 1 : 0);

        nodesGeo.setHeader(0 * 4, nodeGeoEntryBytes);
        nodesGeo.setHeader(1 * 4, Helper.degreeToInt(bounds.minLon));
        nodesGeo.setHeader(2 * 4, Helper.degreeToInt(bounds.maxLon));
        nodesGeo.setHeader(3 * 4, Helper.degreeToInt(bounds.minLat));
        nodesGeo.setHeader(4 * 4, Helper.degreeToInt(bounds.maxLat));
        nodesGeo.setHeader(5 * 4, withElevation ? 1 : 0);
        if (withElevation) {
            nodesGeo.setHeader(6 * 4, Helper.eleToUInt(bounds.minEle));
            nodesGeo.setHeader(7 * 4, Helper.eleToUInt(bounds.maxEle));
        }
    ...
    public long getCapacity() {
        return nodes.getCapacity() + nodesGeo.getCapacity() + edges.getCapacity();
    }

    public boolean isClosed() {
        assert nodes.isClosed() == edges.isClosed();
        assert nodes.isClosed() == nodesGeo.isClosed();
    ...
    public int edge(int nodeA, int nodeB) {
        ...
        // list of edges.
        long nodePointerA = toNodesPointer(nodeA);
        ...
        long nodePointerB = toNodesPointer(nodeB);
    ...
    public void ensureNodeCapacity(int node) {
        ...
        for (int n = oldNodes; n < nodeCount; ++n) {
            setEdgeRef(toNodesPointer(n), NO_EDGE);
            if (withTurnCosts)
                setTurnCostRef(toNodesPointer(n), TurnCostStorage.NO_TURN_ENTRY);
        }
        nodesGeo.ensureCapacity((long) nodeCount * nodeGeoEntryBytes);
    }

    public long toNodesPointer(int node) {
        if (node < 0 || node >= nodeCount)
            throw new IllegalArgumentException("node: " + node + " out of bounds [0," + nodeCount + "[");
        return (long) node * nodeEntryBytes;
    }

    public long toNodesGeoPointer(int node) {
        if (node < 0 || node >= nodeCount)
            throw new IllegalArgumentException("node: " + node + " out of bounds [0," + nodeCount + "[");
        return (long) node * nodeGeoEntryBytes;
    }
    ...
    public void setLat(long nodePointer, double lat) {
        nodesGeo.setInt(nodePointer + N_LAT, Helper.degreeToInt(lat));
    }

    public void setLon(long nodePointer, double lon) {
        nodesGeo.setInt(nodePointer + N_LON, Helper.degreeToInt(lon));
    }

    public void setEle(long nodePointer, double ele) {
        nodesGeo.setInt(nodePointer + N_ELE, Helper.eleToUInt(ele));
    }

    public double getLat(long nodePointer) {
        return Helper.intToDegree(nodesGeo.getInt(nodePointer + N_LAT));
    }

    public double getLon(long nodePointer) {
        return Helper.intToDegree(nodesGeo.getInt(nodePointer + N_LON));
    }

    public double getEle(long nodePointer) {
        return Helper.uIntToEle(nodesGeo.getInt(nodePointer + N_ELE));
    }
    ...
    public void debugPrint() {
        ...
        for (int i = 0; i < Math.min(nodeCount, printMax); ++i) {
            System.out.format(Locale.ROOT, formatNodes, i, getEdgeRef(toNodesPointer(i)), getLat(toNodesGeoPointer(i)), getLon(toNodesGeoPointer(i)));
        }
    ...
    public String toDetailsString() {
        return "edges: " + nf(edgeCount) + "(" + edges.getCapacity() / Helper.MB + "MB), "
                ...
                + "nodesGeo: " + nf(nodeCount) + "(" + nodesGeo.getCapacity() / Helper.MB + "MB), "
                + "bounds: " + bounds;
    }
}
