...
avoid_snap       | -       | Optional parameter to avoid snapping to a certain road class or environment. Current supported values: `motorway`, `trunk`, `ferry`, `tunnel`, `bridge` and `ford`. Multiple values are specified like `avoid_snap=ferry&avoid_snap=motorway`
