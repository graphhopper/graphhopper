 ...
 */
class OSMMtbRatingParser(private val mtbRatingEnc: IntEncodedValue) : TagParser {

    override fun handleWayTags(edgeId: Int, edgeIntAccess: EdgeIntAccess, way: ReaderWay, relationFlags: IntsRef?) {
        var scale = way.getTag("mtb:scale")
        var rating = 0
        if (scale != null) {
            if (scale.length == 2 && (scale[1] == '+' || scale[1] == '-'))
                scale = scale.substring(0, 1)
            try {
                val scaleAsInt = scale.toInt()
                rating = scaleAsInt + 1
            } catch (ex: Exception) {
            }
        }
