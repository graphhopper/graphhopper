...
class TripFromLabel {
    ...
    ResponsePath createResponsePath(Translation tr, PointList waypoints, Graph queryGraph, Weighting connectingWeighting, PtWeighting ptWeighting, List<Label.Transition> solution, List<String> requestedPathDetails, String connectingVehicle, boolean includeElevation) {
        final List<List<Label.Transition>> partitions = parsePathToPartitions(solution);

        final List<Trip.Leg> legs = new ArrayList<>();
        for (int i = 0; i < partitions.size(); i++) {
            legs.addAll(parsePartitionToLegs(partitions.get(i), queryGraph, connectingWeighting, ptWeighting, tr, requestedPathDetails, connectingVehicle, includeElevation));
        }
    ...
    private List<Trip.Leg> parsePartitionToLegs(List<Label.Transition> path, Graph graph, Weighting weighting, PtWeighting ptWeighting, Translation tr, List<String> requestedPathDetails, String connectingVehicle, boolean includeElevation) {
        if (path.size() <= 1) {
                ...
                if (EnumSet.of(GtfsStorage.EdgeType.TRANSFER, GtfsStorage.EdgeType.LEAVE_TIME_EXPANDED_NETWORK).contains(edge.getType())) {
                    ...
                    if (edge.getType() == GtfsStorage.EdgeType.TRANSFER) {
                        ...
                        if (skippedEdgesForTransfer != null) {
                            ...
                            List<Trip.Leg> legs = parsePartitionToLegs(transferPath(skippedEdgesForTransfer, weighting, ptWeighting, path.get(i - 1).label.currentTime, isBike), graph, weighting, ptWeighting, tr, requestedPathDetails, connectingVehicle, includeElevation);
