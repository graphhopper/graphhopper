...
public class FastestWeighting extends AbstractWeighting {
    ...
    public FastestWeighting(FlagEncoder encoder, PMap map, TurnCostProvider turnCostProvider) {
        ...
        if (!encoder.hasEncodedValue(encoder.getTransportationMode().getAccessName()))
            throw new IllegalArgumentException(encoder.getTransportationMode() + " is not available but expected for FastestWeighting");
        roadAccessEnc = destinationPenalty > 1 || privatePenalty > 1 ? encoder.getEnumEncodedValue(encoder.getTransportationMode().getAccessName(), RoadAccess.class) : null;
    }
