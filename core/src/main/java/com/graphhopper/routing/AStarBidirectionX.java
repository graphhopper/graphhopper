 ...
 * @author jansoe
 */
public class AStarBidirectionX extends AbstractNonCHBidirAlgoX {
    private BalancedWeightApproximator weightApprox;
    double stoppingCriterionOffset;

    public AStarBidirectionX(Graph graph, Weighting weighting, TraversalMode tMode) {
    ...
    @Override
    protected SPTEntryX createStartEntry(int node, double weight, double distance, long time, boolean reverse) {
        double heapWeight = weight + weightApprox.approximate(node, reverse);
        return new AStarEntryX(EdgeIterator.NO_EDGE, node, heapWeight, weight, distance, time);
    }

    @Override
    protected SPTEntryX createEntry(EdgeIteratorState edge, double weight, double distance, long time, SPTEntryX parent, boolean reverse) {
        int neighborNode = edge.getAdjNode();
        double heapWeight = weight + weightApprox.approximate(neighborNode, reverse);
        return new AStarEntryX(edge.getEdge(), neighborNode, heapWeight, weight, distance, time, parent);
    }

    @Override
    protected double calcWeight(EdgeIteratorState iter, SPTEntryX currEdge, boolean reverse) {
    ...
    public AStarBidirectionX setApproximation(WeightApproximator approx) {
    ...
    public static class AStarEntryX extends SPTEntryX {
        double weightOfVisitedPath;

        public AStarEntryX(int edgeId, int adjNode, double weightForHeap, double weightOfVisitedPath, double distance, long time) {
            this(edgeId, adjNode, weightForHeap, weightOfVisitedPath, distance, time, null);
        }

        public AStarEntryX(int edgeId, int adjNode, double weightForHeap, double weightOfVisitedPath, double distance, long time, SPTEntryX parent) {
            super(edgeId, adjNode, weightForHeap, distance, time, parent);
