...
public class DirectionResolver {
    ...
    public DirectionResolverResult resolveDirections(int node, GHPoint location) {
        ...
        Node snappedPoint = new Node(nodeAccess.getLat(node), nodeAccess.getLon(node));
        ...
        if (adjacentEdges.nextPoints.size() == 1) {
            Node neighbor = adjacentEdges.nextPoints.iterator().next();
            ...
            Iterator<Node> iter = adjacentEdges.nextPoints.iterator();
            Node p1 = iter.next();
            Node p2 = iter.next();
            ...
            Node locationNode = new Node(location.lat, location.lon);
            if (in1.isEmpty() || out2.isEmpty()) {
                return resolveDirections(snappedPoint, locationNode, in2.get(0), out1.get(0));
            } else if (in2.isEmpty() || out1.isEmpty()) {
                return resolveDirections(snappedPoint, locationNode, in1.get(0), out2.get(0));
            } else {
                return resolveDirections(snappedPoint, locationNode, in1.get(0), out2.get(0), in2.get(0).edgeId, out1.get(0).edgeId);
            }
        } else {
    ...
    private DirectionResolverResult resolveDirections(Node snappedPoint, Node queryPoint, Edge inEdge, Edge outEdge) {
    ...
    private DirectionResolverResult resolveDirections(Node snappedPoint, Node queryPoint, Edge inEdge, Edge outEdge, int altInEdge, int altOutEdge) {
        Node inPoint = inEdge.nextPoint;
        Node outPoint = outEdge.nextPoint;
    ...
    private boolean isOnRightLane(Node queryPoint, Node snappedPoint, Node inPoint, Node outPoint) {
    ...
    private double diffLon(Node p, Node q) {
        return q.lon - p.lon;
    }

    private double diffLat(Node p, Node q) {
        return q.lat - p.lat;
    }
    ...
    private AdjacentEdges calcAdjEdges(int node) {
        ...
        while (iter.next()) {
            ...
            Node nextPoint = new Node(nextPointLat, nextPointLon);
    ...
    private static class AdjacentEdges {
        private final Map<Node, List<Edge>> inEdgesByNextPoint = new HashMap<>(2);
        private final Map<Node, List<Edge>> outEdgesByNextPoint = new HashMap<>(2);
        final Set<Node> nextPoints = new HashSet<>(2);
        int numLoops;
        int numNonLoops;
        ...
        List<Edge> getInEdges(Node p) {
            List<Edge> result = inEdgesByNextPoint.get(p);
            return result == null ? Collections.emptyList() : result;
        }

        List<Edge> getOutEdges(Node p) {
        ...
        private static void addEdge(Map<Node, List<Edge>> edgesByNextPoint, Edge edge) {
    ...
    private static class Node {
        ...
        @Override
        public int hashCode() {
            // it does not matter, because we only use maps with very few elements
            return 0;
        }
    }
    ...
    private static class Edge {
         ...
         * next pillar (!) node.
         */
        final Node nextPoint;

        Edge(int edgeId, int adjNode, Node nextPoint) {
            this.edgeId = edgeId;
