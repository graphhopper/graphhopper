...
enum class Cycleway {
    ...
    companion object {
        const val KEY = "cycleway"

        @JvmStatic
        fun create(): EnumEncodedValue<Cycleway> = EnumEncodedValue(KEY, Cycleway::class.java, true)

        @JvmStatic
        fun find(name: String?): Cycleway {
            if (Helper.isEmpty(name))
                return MISSING
            when (name) {
                "sidepath" -> return SEPARATE
                "crossing" -> return TRACK
                "share_busway", "shared" -> return SHARED_LANE
            }
            return try {
