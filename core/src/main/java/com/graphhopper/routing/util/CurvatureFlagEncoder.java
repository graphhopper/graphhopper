...
/**
 * Defines bit layout for Curvature Routings
 * <p>
 * @author Peter Karich
 */
public class CurvatureFlagEncoder extends CarFlagEncoder
{
    ...
    private HashMap<Integer, Integer> curvatureData;

    public CurvatureFlagEncoder()
    {
        this(5, 5, 0);
    }

    public CurvatureFlagEncoder(PMap properties)
    {
        this(
    ...
    public CurvatureFlagEncoder(String propertiesStr)
    {
        this(new PMap(propertiesStr));
    }

    public CurvatureFlagEncoder(int speedBits, double speedFactor, int maxTurnCosts)
    {
        super(speedBits, speedFactor, maxTurnCosts);

        //Init Curvature Storage
        curvatureData = new HashMap<Integer, Integer>();
    ...
    public void applyWayTags( OSMWay way, EdgeIteratorState edge )
    {
        ...
        double straightLine = Double.MAX_VALUE;

        try{
            straightLine = Double.parseDouble(way.getTag("straight_line"));
        }catch (Exception e){
            System.out.println("Could not parse the String to Double");
            //System.out.println("Could not parse the String: "+ String.valueOf(Double.parseDouble(way.getTag("straight_line"))) +" to Double");
        }


        int curvature = (int) (speed * (distance/straightLine));

        System.out.println("Calculated the curvature of: "+curvature+" using the speed "+speed+" the distance "+distance+" the straightLine"+ straightLine);

        curvatureData.put(edge.getEdge(), curvature);
    }

    public int getCurvature(EdgeIteratorState edge){
        return curvatureData.getOrDefault(edge.getEdge(), 1);
    }
