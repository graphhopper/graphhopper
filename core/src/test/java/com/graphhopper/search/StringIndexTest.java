...
public class StringIndexTest {
    ...
    private StringIndex create() {
        return new StringIndex(new RAMDirectory(), "string_index").create(1000);
    }
    ...
    public void testFlush() {
        ...
        StringIndex index = new StringIndex(new RAMDirectory(location, true).create(), "string_index").create(1000);
        long pointer = index.add(createMap("", "test"));
        index.flush();
        index.close();

        index = new StringIndex(new RAMDirectory(location, true), "string_index");
