...
//        ));
    ...
    public MiniGraphUI(GraphHopper hopper, boolean debug, boolean useCH) {
        ...
        mg.scale(500, 300, true);
        mg.scale(500, 300, true);
        mg.scale(500, 200, true);
        mg.scale(500, 200, true);
        ...
        mainPanel.addLayer(pathLayer = new DefaultMapLayer() {
            ...
            public void paintComponent(final Graphics2D g2) {
//                 **** overwrite
//                double fromLat = 52.468821, fromLon = 13.319261, toLat = 52.495914, toLon = 13.400179;
    ...
    private RoutingAlgorithm createAlgo(GraphHopper hopper, QueryGraph qGraph) {
        ...
        if (useCH) {
            ...
            RoutingAlgorithmFactory algoFactory = (g, w, opts) -> {
                ...
                RoutingAlgorithm algo = new RoutingAlgorithmFactorySimple().createAlgo(g, w, opts);
                if (algo instanceof AlternativeRoute) {
                    return new DebugAlternativeRoute(g, w, opts.getTraversalMode(), 1.5, mg);
            ...
            AlgorithmOptions algoOpts = new AlgorithmOptions().setAlgorithm(Algorithms.ALT_ROUTE).setTraversalMode(TraversalMode.EDGE_BASED);
                    ...
                    MouseAdapter ml = new MouseAdapter() {
                        ...
                        public void mouseClicked(MouseEvent e) {
                            if (!fromDone) {
                                ...
                                fromRes = index.findClosest(fromLat, fromLon, EdgeFilter.ALL_EDGES);
                                toRes = index.findClosest(toLat, toLon, EdgeFilter.ALL_EDGES);
                                if (!fromRes.isValid()) fromRes = null;
                                if (!toRes.isValid()) toRes = null;
