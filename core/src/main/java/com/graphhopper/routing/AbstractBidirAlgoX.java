...
public abstract class AbstractBidirAlgoX implements EdgeToEdgeRoutingAlgorithm {
    protected final TraversalMode traversalMode;
    protected int from;
    protected int to;
    protected int fromOutEdge;
    protected int toInEdge;
    protected IntObjectMap<SPTEntryX> bestWeightMapFrom;
    protected IntObjectMap<SPTEntryX> bestWeightMapTo;
    protected IntObjectMap<SPTEntryX> bestWeightMapOther;
    protected SPTEntryX currFrom;
    protected SPTEntryX currTo;
    protected SPTEntryX bestFwdEntry;
    protected SPTEntryX bestBwdEntry;
    ...
    PriorityQueue<SPTEntryX> pqOpenSetFrom;
    PriorityQueue<SPTEntryX> pqOpenSetTo;
    ...
    public AbstractBidirAlgoX(TraversalMode traversalMode) {
     ...
     */
    protected abstract SPTEntryX createStartEntry(int node, double weight, double distance, long time, boolean reverse);
    ...
    void init(int from, double fromWeight, int to, double toWeight) {
        initFrom(from, fromWeight, 0, 0);
        initTo(to, toWeight, 0, 0);
        postInit(from, to);
    }

    protected void initFrom(int from, double weight, double distance, long time) {
        this.from = from;
        currFrom = createStartEntry(from, weight, distance, time, false);
    ...
    protected void initTo(int to, double weight, double distance, long time) {
        this.to = to;
        currTo = createStartEntry(to, weight, distance, time, true);
    ...
    protected void updateBestPath(double edgeWeight, SPTEntryX entry, int origEdgeIdForCH, int traversalId, boolean reverse) {
        assert traversalMode.isEdgeBased() != Double.isInfinite(edgeWeight);
        SPTEntryX entryOther = bestWeightMapOther.get(traversalId);
        if (entryOther == null)
            return;
    ...
    protected abstract double getInEdgeWeight(SPTEntryX entry);

    protected int getIncomingEdge(SPTEntryX entry) {
        return entry.edge;
    }
    ...
    IntObjectMap<SPTEntryX> getBestFromMap() {
        return bestWeightMapFrom;
    }

    IntObjectMap<SPTEntryX> getBestToMap() {
        return bestWeightMapTo;
    }

    void setBestOtherMap(IntObjectMap<SPTEntryX> other) {
    ...
    void setToDataStructures(AbstractBidirAlgoX other) {
        to = other.to;
