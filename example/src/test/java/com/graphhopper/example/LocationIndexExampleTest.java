...
class LocationIndexExampleTest {
    ...
    @Test
    public void graphhopperLocationIndex() {
        GraphHopper hopper = new GraphHopperOSM().setEncodingManager(EncodingManager.create(new CarFlagEncoder()));
        ...
        assertEquals("Avinguda Meritxell", edge.getName());
    }

    @Test
    public void lowLevelLocationIndex() {
        ...
        EdgeIteratorState edge = snap.getClosestEdge();
        assertEquals("test edge", edge.getName());
    }
}