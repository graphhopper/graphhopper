...
public class OSMReader implements TurnCostParser.ExternalInternalMap {
    ...
    private void splitWayAtJunctionsAndEmptySections(ReaderWay way, IntsRef edgeFlags) {
        ...
        for (LongCursor node : way.getNodes()) {
            int id = getNodeMap().get(node.value);
            if (!isNodeId(id)) {
                ...
                if (segment.size() > 1) {
                    splitLoops(segment, way, edgeFlags);
    ...
    private void splitLoops(LongArrayList segment, ReaderWay way, IntsRef edgeFlags) {
        if (segment.size() < 2)
        ...
        if (segment.size() == 2 && isLoop) {
            ...
            splitSegmentAtSplitNodes(segment1, way, edgeFlags);
            LongArrayList segment2 = new LongArrayList(segment.size());
            segment2.add(segment.buffer, segment.size() - 2, 2);
            splitSegmentAtSplitNodes(segment2, way, edgeFlags);
        } else {
            splitSegmentAtSplitNodes(segment, way, edgeFlags);
        }
    }

    private void splitSegmentAtSplitNodes(LongArrayList parentSegment, ReaderWay way, IntsRef edgeFlags) {
        LongArrayList segment = new LongArrayList();
        for (LongCursor node : parentSegment) {
            int nodeTagIndex = nodeTagIndicesByOsmNodeID.get(node.value);
            if (nodeTagIndex >= 0) {
                // this node is a barrier. we will add an extra edge to be able to block access
                long barrierFrom = node.value;
                long barrierTo = addBarrierNode(barrierFrom);
                ...
                nodeTagIndicesByOsmNodeID.put(node.value, -1);
    ...
    void handleSegment(LongArrayList segment, ReaderWay way, IntsRef edgeFlags, Map<String, Object> nodeTags) {
        ...
        for (LongCursor node : segment) {
            ...
            if (isPillarNode(id)) {
                ...
                boolean convertToTowerNode = node.index == 0 || node.index == segment.size() - 1;
                id = handlePillarNode(id, node.value, pointList, convertToTowerNode);
            }
