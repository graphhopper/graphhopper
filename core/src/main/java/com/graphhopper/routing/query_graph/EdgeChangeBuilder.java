...
/**
 * Helper class for {@link VirtualEdgeBuilder}
 *
...
class EdgeChangeBuilder {
    ...
    private final IntObjectMap<QueryGraphModification.EdgeChanges> edgeChangesAtRealNodes;
     ...
     *                               be added to it
     */
    static void build(IntArrayList closestEdges, List<VirtualEdgeIteratorState> virtualEdges, int firstVirtualNodeId, IntObjectMap<QueryGraphModification.EdgeChanges> edgeChangesAtRealNodes) {
        new EdgeChangeBuilder(closestEdges, virtualEdges, firstVirtualNodeId, edgeChangesAtRealNodes).build();
    }

    private EdgeChangeBuilder(IntArrayList closestEdges, List<VirtualEdgeIteratorState> virtualEdges, int firstVirtualNodeId, IntObjectMap<QueryGraphModification.EdgeChanges> edgeChangesAtRealNodes) {
    ...
    private void addVirtualEdges(boolean base, int node, int virtNode) {
        QueryGraphModification.EdgeChanges edgeChanges = edgeChangesAtRealNodes.get(node);
        if (edgeChanges == null) {
            edgeChanges = new QueryGraphModification.EdgeChanges(2, 2);
    ...
    private void addRemovedEdges(int towerNode) {
        ...
        QueryGraphModification.EdgeChanges edgeChanges = edgeChangesAtRealNodes.get(towerNode);
