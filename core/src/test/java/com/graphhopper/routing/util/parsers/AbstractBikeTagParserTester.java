...
public abstract class AbstractBikeTagParserTester {
    ...
    private void assertAccess(ReaderWay way, boolean fwd, boolean bwd) {
        ...
        accessParser.handleWayTags(edgeFlags, way, relationFlags);
        if (fwd) assertTrue(accessEnc.getBool(false, edgeFlags));
        if (bwd) assertTrue(accessEnc.getBool(true, edgeFlags));
    }
}
