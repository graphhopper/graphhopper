...
public class GraphEdgeIdFinderTest {
    ...
    public void testBlockAreasWithPolygon() {
        ...
        GraphEdgeIdFinder.BlockArea blockArea = graphFinder.parseBlockArea("2.1,1, -1.1,2, 2,3", DefaultEdgeFilter.allEdges(encoder.getAccessEnc()), area);
        ...
        blockArea = graphFinder.parseBlockArea("2.1,1, 0.9,3, 0.9,2, -0.3,0", DefaultEdgeFilter.allEdges(encoder.getAccessEnc()), 1000 * 1000);
        assertFalse(blockArea.hasCachedEdgeIds(0));
        assertEdges(graph, "[0, 1, 4, 5, 6, 7, 9, 10]", blockArea);

        blockArea = graphFinder.parseBlockArea("1.5,3,100000", DefaultEdgeFilter.allEdges(encoder.getAccessEnc()), area);
