...
public class GHLongLongBTree implements LongLongMap
{
    ...
    public GHLongLongBTree( int maxLeafEntries )
    {
...
//        values.flush();
    ...
    /**
     * Apply visitor pattern to all keys in the tree.
     * @param visitor
     */
    ...
    static class ReturnValue
    {
        ...
        public ReturnValue( long oldValue )
        {
    ...
    class BTreeEntry
    {
        ...
        ReturnValue put( long key, long newValue )
        {
            ...
            // safeguard against accidential updates during visitor execution
            if( locked )
                throw new IllegalStateException("Tree cannot be extended while locked by visitor.");
        ...
        void insertKeyValue( int index, long key, long newValue )
        {
        ...
        long getCapacity()
        {
            long cap = keys.length * (8 + 8) + 3 * 12 + 4 + 1;
            if (!isLeaf)
            {
        ...
        public void apply( Visitor visitor )
        {
            for (int i = 0; i < entrySize; i++)
            {
                if (!isLeaf)
                {
                    children[i].apply( visitor );
                }
                visitor.processKey( keys[i]);
            }
            if (!isLeaf)
            {
                children[entrySize].apply( visitor );
            }
        }
    }
