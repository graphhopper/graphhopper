 ...
 * @author Peter Karich
 */
object EdgeElevationSmoothingRamer {

    private val LOGGER: Logger = LoggerFactory.getLogger(EdgeElevationSmoothingRamer::class.java)
     ...
     */
    @JvmStatic
    fun smooth(pointList: PointList, maxElevationDelta: Double) {
        internSmooth(pointList, 0, pointList.size() - 1, maxElevationDelta, DistanceCalcEarth.calcDistance(pointList, false), 0)
    }

    internal fun internSmooth(pointList: PointList, fromIndex: Int, lastIndex: Int, maxElevationDelta: Double, fullDist2D: Double, depth: Int) {
        ...
        var prevLat = pointList.getLat(fromIndex)
        var prevLon = pointList.getLon(fromIndex)
        ...
        val averageSlope = if (fullDist2D == 0.0) 0.0 else (pointList.getEle(lastIndex) - pointList.getEle(fromIndex)) / fullDist2D
        var prevAverageSlopeEle = pointList.getEle(fromIndex)
        var startDist = 0.0
        var maxEleDelta = -1.0
        var indexWithMaxDelta = -1
        for (i in fromIndex + 1 until lastIndex) {
            val lat = pointList.getLat(i)
            val lon = pointList.getLon(i)
            val ele = pointList.getEle(i)
            val tmpDist2D = DistanceCalcEarth.DIST_EARTH.calcDist(prevLat, prevLon, lat, lon)
            startDist += tmpDist2D
            val eleFromAverageSlope = averageSlope * tmpDist2D + prevAverageSlopeEle
            val tmpEleDelta = Math.abs(ele - eleFromAverageSlope)
        ...
        if (indexWithMaxDelta < 0 || maxElevationDelta > maxEleDelta) {
            ...
            internSmooth(pointList, indexWithMaxDelta, lastIndex, maxElevationDelta, Math.max(0.0, fullDist2D - startDist), depth + 1)
        }
    }
}
