/*
 * Kotlin port of com.carrotsearch.hppc.LongLongHashMap from HPPC 0.8.1 (Apache License 2.0,
...
/**
 * A hash map of `long` to `long` — a 1:1 layout port of HPPC 0.8.1's `LongLongHashMap` with
 ...
 */
open class LongLongHashMap @JvmOverloads constructor(
     ...
     */
    fun putOrAdd(key: Long, putValue0: Long, incrementValue: Long): Long {
     ...
     */
    fun addTo(key: Long, incrementValue: Long): Long = putOrAdd(key, incrementValue, incrementValue)
    ...
    fun indexInsert(index0: Int, key: Long, value: Long) {
    ...
    inline fun forEachWhile(predicate: (key: Long, value: Long) -> Boolean) {
        val keys = this.keys
        val values = this.values
    ...
    inline fun forEachInIteratorOrder(action: (key: Long, value: Long) -> Unit) {
        val keys = this.keys
        val values = this.values
        val max = this.mask
        var slot = 0
    ...
    override fun equals(other: Any?): Boolean {
        return other != null &&
                ...
                equalElements(other as LongLongHashMap)
    }

    protected fun equalElements(other: LongLongHashMap): Boolean {
    ...
    protected fun rehash(fromKeys: LongArray, fromValues: LongArray) {
        val keys = this.keys
        val values = this.values
        val mask = this.mask
    ...
    protected fun allocateThenInsertThenRehash(slot: Int, pendingKey: Long, pendingValue: Long) {
        val prevKeys = this.keys
        val prevValues = this.values
