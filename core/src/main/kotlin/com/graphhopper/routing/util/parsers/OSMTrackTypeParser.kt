...
class OSMTrackTypeParser(private val trackTypeEnc: EnumEncodedValue<TrackType>) : TagParser {

    override fun handleWayTags(edgeId: Int, edgeIntAccess: EdgeIntAccess, way: ReaderWay, relationFlags: IntsRef?) {
        val trackTypeTag = way.getTag("tracktype")
        val trackType = TrackType.find(trackTypeTag)
        if (trackType != TrackType.MISSING)
