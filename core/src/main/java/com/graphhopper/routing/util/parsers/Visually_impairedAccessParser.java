...
public class Visually_impairedAccessParser extends AbstractAccessParser implements TagParser {
    ...
    public WayAccess getAccess(ReaderWay way) {
        ...
        if (highwayValue == null) {
            ...
            if (FerrySpeedCalculator.isFerry(way)) {
                String visually_impairedTag = way.getTag("foot");
                if (visually_impairedTag == null || intendedValues.contains(visually_impairedTag))
    ...
    public void handleWayTags(int edgeId, EdgeIntAccess edgeIntAccess, ReaderWay way) {
        ...
        if (way.hasTag("oneway:visually_impaired", oneways) || way.hasTag("visually_impaired:backward") || way.hasTag("visually_impaired:forward")
                || way.hasTag("oneway", oneways) && way.hasTag("highway", "steps") // outdated mapping style
        ) {
            boolean reverse = way.hasTag("oneway:visually_impaired", "-1") || way.hasTag("visually_impaired:backward", "yes") || way.hasTag("visually_impaired:forward", "no");
