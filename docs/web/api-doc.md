...
:----------------|:--------------------------|:--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
 ...
 points_encoded_multiplier | 1e5                       | Used in case `points_encoded=true` to encode the `points` string into an array of coordinates.
 ...
 snap_prevention | `[tunnel, bridge, ferry]` | 'Snapping' is the process of finding the closest road location for GPS coordinates provided in the `point` parameter. The `snap_prevention` parameter allows you to prevent snapping to specific types of roads. For example, if `snap_prevention` is set to bridge, the routing engine will avoid snapping to a bridge, even if it is the closest road for the given `point`. Current supported values: `motorway`, `trunk`, `ferry`, `tunnel`, `bridge`, `ford` and `access_control_restricted`. Multiple values are specified like `snap_prevention=ferry&snap_prevention=motorway`. Note that once snapped the routing algorithm can still route over bridges (or the other values). To avoid this you need to use the `custom_model`. The `access_control_restricted` option prevents snapping to roads with full access control (like freeways) including those explicitly tagged with `access_control=full` on OpenStreetMap.
 ...
 via_point_instructions           | true                     | Optional parameter. If true, the instructions will contain via-point instructions such as "Reached waypoint X".
